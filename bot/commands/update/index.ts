  ...
  for (const cycle of uncompletedCycles) {
    ...
    if (status === Status.ORDER_BUY_PLACED) {
      ...
      if (order.isActive) {
        ...
        Cycle.updateOrderSellId(Number(cycleId), String(orderSell.id));

        Cycle.updateStatus(Number(cycleId), Status.ORDER_SELL_PLACED);
      }
      ...
      if (order.isActive) {
        ...
        Cycle.updateStatus(Number(cycleId), Status.COMPLETED);
